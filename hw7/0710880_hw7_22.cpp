...
int main()
{
    ...
    BST_TREE *root;
    root = BST_Create(compareint);

     while(!infile.eof())
    {
        personal_imfo *data = new personal_imfo;
        infile>>data->name>>data->phone_number;

        BST_Insert(root,data);
    }

    BST_Traverse(root,printBST);
...
void printBST(void* data)
{
    cout<<((personal_imfo*)data)->name<<"\t"<<((personal_imfo*)data)->phone_number<<endl;
    return;
}
